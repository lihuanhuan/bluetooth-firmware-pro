...
ret_code_t axp216_twi_master_init(void) {
  ...
  const nrf_drv_twi_config_t axp216_twi_config = {
      .scl = AXP216_TWI_SCL_M, .sda = AXP216_TWI_SDA_M, .frequency = NRF_DRV_TWI_FREQ_400K, .interrupt_priority = APP_IRQ_PRIORITY_HIGH, .clear_bus_init = false};
...
void axp_disable(void) { nrf_drv_twi_disable(&axp216_m_twi); }
